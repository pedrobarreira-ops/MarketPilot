...
test.describe('Report page (public/report.html served at /report/:id)', () => {
  ...
  // ── UNSKIP when Story 6.1 ships report.js ─────────────────────────────────
  // AC: skeleton state shown while GET /api/reports/:id is in flight (shimmer cards + rows)
  test.skip('6.1 — skeleton shows while report fetch is in flight', async ({ page }) => {
    ...
    // Skeleton shimmer visible — assert per UX-DR9 / UX-DR10
  ...
  test.skip('6.1 — populated state shows PT stat cards with pt-PT locale numbers', async ({ page }) => {
    ...
    // Uses shared helper — asserts "4.821" (not "4821"), "1.340", "756"
    // const { expectPortugueseInteger } = await import('./test-helpers.js')
    // await expectPortugueseInteger(page.getByText(/em 1\.º lugar/i).locator('..'), 4821)
    // await expectPortugueseInteger(page.getByText(/a perder posição/i).locator('..'), 1340)
  })

  // ── UNSKIP when Story 6.1 ships report.js (PT/ES toggle) ───────────────
  // AC: clicking ES pill swaps data; clicking PT pill swaps back; aria-pressed updated
  test.skip('6.1 — PT/ES toggle swaps data without re-fetch', async ({ page }) => {
    let fetchCount = 0
    await page.route(`**/api/reports/${SAMPLE_ID}`, (route) => {
      fetchCount++
      route.fulfill({
    ...
    await page.goto(`/report/${SAMPLE_ID}`)
    // ... click ES toggle, assert data swapped, assert fetchCount still 1 ...
  })
